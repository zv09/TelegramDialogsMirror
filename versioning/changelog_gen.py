...
def generate_changelog_content(latest_tag, current_version):
    ...
    if latest_tag:
        ...
        command = f"git-changelog --output - {latest_tag}..HEAD"
    else:
    ...
    try:
        ...
        print(f"Error generating changelog: {e.stderr}")
        return ""
...
def update_changelog_file(new_content, current_version):
    ...
    try:
        ...
        print(f"Updated {changelog_path} with version {current_version} changelog.")

    except FileNotFoundError:
        print(f"Error: {changelog_path} not found.")
    except Exception as e:
        print(f"An error occurred while updating changelog: {e}")
...
if __name__ == "__main__":
    ...
    if not current_version:
        print("Could not determine current version from pyproject.toml. Exiting.")
    else:
        print(f"Generating changelog for version {current_version} from tag {latest_tag if latest_tag else 'beginning of history'}...")
